...
export const ConsecutiveSteady = () => {
...
2.  **المحتوى المطلوب:**
    * ادرس اطراد المتتالية (Un)n>=0: ${cons} (متزايدة أو متناقصة أو ثابتة) وفق أحد هذه المعايير الثلاثة: 1: معيار الفرق Un+1 - Un, 2: معيار النسبة: Un+1/Un 3: معيار التابع (أي يمكن كتاية المتتالية بصيغة تابع ودراسة اطراده, استعمل المعيار الأسهل (لا داعي لتحلها بكل المعايير), مهم: إن أمكن كتابتها بصيغة تابع حلها باستخدام معيار التابع حتى إن لم يكن الأسهل`
      );
    }
  }, [cons, requestAgain]);
  ...
  return (
    <div className="container section-padding func-changes-container">
      ...
      <div className="shortcuts fade-in fade-in-3">
        ...
        <button
          onClick={() => {
            consecutiveRef.current.value = consecutiveRef.current.value + "ⁿ";
            consecutiveRef.current.focus();
          }}
        >
          ⁿ
